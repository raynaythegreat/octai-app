...
//  3. ~/.ssh/octai_ed25519.key (os.UserHomeDir is cross-platform)
package credential
...
// octaiHome is a package-local copy of config.EnvHome. It is kept here to
// avoid a circular import between pkg/credential and pkg/config.
const octaiHome = "OCTAI_HOME"
...
// OCTAI_SSH_KEY_PATH env var or ~/.ssh/octai_ed25519.key.
...
func allowedSSHKeyPath(path string) bool {
	...
	// Within OCTAI_HOME.
	if picoHome := os.Getenv(octaiHome); picoHome != "" {
	...
	if sshKeyPath == "" {
		return nil, fmt.Errorf(
			"credential: SSH private key is required but not found" +
				" (set OCTAI_SSH_KEY_PATH or place key at ~/.ssh/octai_ed25519.key)")
	}
...
//  2. OCTAI_SSH_KEY_PATH env var
//  3. ~/.ssh/octai_ed25519.key (auto-detection)
//
